...
for student in studentsArray{
	...
	studentDict.updateValue(pair[1], forKey: pair[0] )
...
for group in studentsGroups{
	...
	for student in Array(group.value){
		var marks:[Int] = []
		for _ in 1...9{
			marks.append(randomValue(maxValue:16))
		}
		buffer.updateValue(marks, forKey: student)
	}
	studentPoints.updateValue(buffer, forKey: group.key)
}
...
for group in studentPoints{
	...
	for student in group.value{
		...
		buffer.updateValue(mark, forKey: student.key)
	}
	sumPoints.updateValue(buffer, forKey: group.key)
}
...
for group in sumPoints{
	...
	var counter : Int = 0
	for student in group.value{
		avMark += Float(student.value)
		counter += 1
		}
	groupAvg.updateValue(avMark/Float(counter), forKey: group.key)
}
...
for group in sumPoints{
	...
	passedPerGroup.updateValue(buffer, forKey: group.key)
}
